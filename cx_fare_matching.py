...
def fbr(inp):
    cpu, num = inp
    curs = myConnection.cursor()
    stmt = 'call rax.cx_fare_matching_fbr(%d,%d)' % (cpu, num)
    curs.execute(stmt)
    curs.close()
...
def fbrs(inp):
    cpu, num = inp
    curs = myConnection.cursor()
    stmt = 'call rax.fare_matching_fbrs(%d,%d)' % (cpu, num)
    curs.execute(stmt)
    curs.close()
...
def ocda(inp):
    cpu, num = inp
    curs = myConnection.cursor()
    stmt = 'call rax.cx_fare_matching_ocda(%d,%d)' % (cpu, num)
    curs.execute(stmt)
    curs.close()
...
def dcda(inp):
    cpu, num = inp
    curs = myConnection.cursor()
    stmt = 'call rax.cx_fare_matching_dcda(%d,%d)' % (cpu, num)
    curs.execute(stmt)
    curs.close()
...
def twocda(inp):
    cpu, num = inp
    curs = myConnection.cursor()
    stmt = 'call rax.cx_fare_matching_2cda(%d,%d)' % (cpu, num)
    curs.execute(stmt)
    curs.close()
...
def spec(inp):
    cpu, num = inp
    curs = myConnection.cursor()
    stmt = 'call rax.cx_fare_matching_spec(%d,%d)' % (cpu, num)
    curs.execute(stmt)
    curs.close()
...
def fm_post_2(inp):
    cpu, num = inp
    curs = myConnection.cursor()
    stmt = 'call rax.cx_fm_post2_r12r2(%d,%d)' % (cpu, num)
    curs.execute(stmt)
    curs.close()
...
if __name__ == '__main__':
    ...
    #
    curs = myConnection.cursor()
    stmt = 'truncate zz_cx.temp_tbl_fc_fare_map_fbr '
    curs.execute(stmt)
    curs.close()

    # curs = myConnection.cursor()
    # stmt = 'truncate zz_cx.temp_tbl_fc_fare_map_fbrs '
    # curs.execute(stmt)
    # curs.close()
    #
    # curs = myConnection.cursor()
    # stmt = 'truncate zz_cx.temp_tbl_fc_fare_map_ocda  '
    # curs.execute(stmt)
    # curs.close()
    #
    # curs = myConnection.cursor()
    # stmt = 'truncate zz_cx.temp_tbl_fc_fare_map_dcda '
    # curs.execute(stmt)
    # curs.close()
    #
    # curs = myConnection.cursor()
    # stmt = 'truncate zz_cx.temp_tbl_fc_fare_map_2cda '
    # curs.execute(stmt)
    # curs.close()
    #
    # curs = myConnection.cursor()
    # stmt = 'truncate zz_cx.temp_tbl_fc_fare_map_spec'
    # curs.execute(stmt)
    # curs.close()

    ## pre_matching
    #
    # msg = 'Processing pre_cx_fm_pre_cmz matching...'
    # FileWrite(msg)
    # start = datetime.now()
    # curs = myConnection.cursor()
    # stmt = "call rax.cx_fm_pre_cmz(1, 1,'cx','201805')"
    # curs.execute(stmt)
    # curs.close()
    # end = datetime.now()
    ...
    # start = datetime.now()
    # curs = myConnection.cursor()
    # stmt = "call rax.cx_fm_pre_static(1, 1,'cx','201805')"
    # curs.execute(stmt)
    # curs.close()
    # end = datetime.now()
    # msg = "Pre_fare_static matching cost %s seconds" % (end - start).total_seconds()
    ...
    # start = datetime.now()
    # curs = myConnection.cursor()
    # stmt = "call rax.cx_fm_pre_dynm(1, 1, 'cx','201805')"
    # curs.execute(stmt)
    # curs.close()
    # end = datetime.now()
    ...
    myConnection = pymysql.connect(host=hostname, user=username, passwd=password, db=database)
    stmt = 'select count(*) from zz_cx.temp_tbl_fc_fare_map_fbr'
    number = mysql(stmt)
    msg = 'fbr generates {0} records'.format(number)
    FileWrite(msg)
    stmt = 'select count(*) from zz_cx.temp_tbl_fc_fare_map_fbrs'
    number = mysql(stmt)
    msg = 'fbrs generates {0} records'.format(number)
    FileWrite(msg)
    stmt = 'select count(*) from zz_cx.temp_tbl_fc_fare_map_spec'
    number = mysql(stmt)
    msg = 'spec generates {0} records'.format(number)
    FileWrite(msg)
    stmt = 'select count(*) from zz_cx.temp_tbl_fc_fare_map_ocda'
    number = mysql(stmt)
    msg = 'ocda generates {0} records'.format(number)
    FileWrite(msg)
    stmt = 'select count(*) from zz_cx.temp_tbl_fc_fare_map_dcda'
    number = mysql(stmt)
    msg = 'dcda generates {0} records'.format(number)
    FileWrite(msg)
    stmt = 'select count(*) from zz_cx.temp_tbl_fc_fare_map_2cda'
    number = mysql(stmt)
    ...
    stmt = 'select count(*) from zz_cx.temp_tbl_fc_fare_matching'
    number = mysql(stmt)
    ...
    FileWrite(msg)
    curs = myConnection.cursor()
    # stmt = "call rax.cx_fm_post3_dump2audit(1, 1, '2018-05-01', '2018-5-31','201805')"
    stmt = "call rax.cx_fm_post3_dump2audit_pu_io('201805')"
    curs.execute(stmt)
    curs.close()
    end = datetime.now()
    msg = "End Time %s"%end
